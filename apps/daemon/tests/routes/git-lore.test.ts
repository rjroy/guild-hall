...
import type { GitLoreDeps } from "@/apps/daemon/routes/git-lore";
import { cleanGitEnv, type GitOps } from "@/apps/daemon/lib/git";
...
describe("createGitOps().commitLore staging boundary", () => {
  ...
  test("commits only .lore/ files, not files outside .lore/", async () => {
    const { createGitOps } = await import("@/apps/daemon/lib/git");
