...
import type { CommissionId, CommissionStatus } from "@/apps/daemon/types";
...
import { CLAUDE_BRANCH } from "@/apps/daemon/lib/git";
...
import {
  ...
  formatTimestamp,
  escapeYamlValue,
} from "@/apps/daemon/lib/toolbox-utils";
import { withProjectLock } from "@/apps/daemon/lib/project-lock";
import type { EventBus } from "@/apps/daemon/lib/event-bus";
import type { CommissionLifecycle } from "@/apps/daemon/services/commission/lifecycle";

import { replaceYamlField } from "@/apps/daemon/lib/record-utils";
import type { CommissionRecordOps } from "@/apps/daemon/services/commission/record";
import type {
  WorkspaceOps,
  FinalizeResult,
} from "@/apps/daemon/services/workspace";
import {
  prepareSdkSession,
  drainSdkSession,
  runSdkSession,
...
export function createCommissionOrchestrator(
  ...
  async function recoverCommissions(): Promise<number> {
    ...
    for (const file of stateJsonFiles) {
      ...
      // Orphaned sleeping state files: mail system removed, transition to failed
  ...
  async function createCommission(
    ...
    const timelineReason = options?.source
...
`;
  ...
  async function dispatchCommission(
    ...
    // toolbox can access the commission session, git ops, and other deps.
