...
import { noopEventBus, type EventBus } from "@/apps/daemon/lib/event-bus";
...
} from "@/apps/daemon/services/manager/worker";
import { buildManagerContext } from "@/apps/daemon/services/manager/context";
import {
  prepareSdkSession,
  type SessionPrepSpec,
  type SessionPrepDeps,
  type SdkQueryOptions,
} from "@/apps/daemon/lib/agent-sdk/sdk-runner";
import type { GuildHallEvent, MeetingId, MeetingStatus, SdkSessionId } from "@/apps/daemon/types";
...
import {
  ...
  removeTranscript,
  truncateTranscript,
} from "@/apps/daemon/services/meeting/transcript";
import {
  getGuildHallHome,
...
} from "@/apps/daemon/services/meeting/notes-generator";
import { isNodeError } from "@/lib/types";
import { loadMemories } from "@/apps/daemon/services/memory-injector";
import { MeetingRegistry, type ActiveMeetingEntry } from "@/apps/daemon/services/meeting/registry";
import type { WorkspaceOps } from "@/apps/daemon/services/workspace";
...
} from "@/apps/daemon/services/meeting/session-loop";

// Re-export for backward compatibility (tests import from orchestrator)
export { MEETING_GREETING_PROMPT } from "@/apps/daemon/services/meeting/session-loop";
...
export function createMeetingSession(deps: MeetingSessionDeps): MeetingSessionForRoutes {
  ...
  async function getWorkspace(): Promise<WorkspaceOps> {
    ...
    const { createWorkspaceOps } = await import("@/apps/daemon/services/workspace");
