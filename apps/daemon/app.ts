...
import { createOperationsRegistry, type OperationsRegistry } from "@/apps/daemon/lib/operations-registry";
import type { AppConfig, DiscoveredPackage, RouteModule, OperationDefinition } from "@/lib/types";
import { createPackageOperationRoutes, type PackageOperationRouteDeps } from "@/apps/daemon/routes/package-operations";
import { loadPackageOperations } from "@/apps/daemon/services/operations-loader";
import { OperationHandlerError } from "@/apps/daemon/services/operation-types";
import { asCommissionId, asMeetingId } from "@/apps/daemon/types";
import type { MeetingSessionDeps } from "@/apps/daemon/services/meeting/orchestrator";
...
import type { SessionPrepDeps } from "@/apps/daemon/lib/agent-sdk/sdk-runner";
import { nullLog } from "@/apps/daemon/lib/log";
import type { CreateLog } from "@/apps/daemon/lib/log";
...
export async function createProductionApp(options?: {
  ...
  const { createMeetingSession } = await import(
    "@/apps/daemon/services/meeting/orchestrator"
  );
  const { MeetingRegistry } = await import(
    "@/apps/daemon/services/meeting/registry"
  );
  const { createCommissionOrchestrator } = await import(
    "@/apps/daemon/services/commission/orchestrator"
  );
  const { createCommissionRecordOps } = await import(
    "@/apps/daemon/services/commission/record"
  );
  const { createCommissionLifecycle } = await import(
    "@/apps/daemon/services/commission/lifecycle"
  );
  const { createWorkspaceOps } = await import(
    "@/apps/daemon/services/workspace"
  );
  const { createEventBus } = await import("@/apps/daemon/lib/event-bus");
  ...
  const { ensureHeartbeatFile } = await import("@/apps/daemon/services/heartbeat/heartbeat-file");
  for (const project of config.projects) {
    const iPath = integrationWorktreePath(guildHallHome, project.name);
    try {
  ...
  // Failures log a warning but don't crash the daemon.
  const { syncProject } = await import("@/apps/daemon/services/git-admin");
  for (const project of config.projects) {
    try {
  ...
  const { createManagerPackage } = await import(
    "@/apps/daemon/services/manager/worker"
  );
  ...
  const { resolveToolSet } = await import("@/apps/daemon/services/toolbox-resolver");
  const { loadMemories } = await import("@/apps/daemon/services/memory-injector");
  const { activateWorker: activateWorkerFn } = await import(
    "@/apps/daemon/services/manager/worker"
  );
  const { createContextTypeRegistry } = await import("@/apps/daemon/services/context-type-registry");
  ...
  const { createBriefingRefreshService } = await import(
    "@/apps/daemon/services/briefing-refresh"
  );
  ...
  const { createHeartbeatService } = await import(
    "@/apps/daemon/services/heartbeat/index"
  );
  ...
  const packageOperations = await loadPackageOperations(allPackages, {
    ...
    transitionCommission: async (commissionId, transition, payload) => {
      ...
      const methodMap: Record<string, () => Promise<import("@/apps/daemon/services/commission/lifecycle").TransitionResult>> = {
