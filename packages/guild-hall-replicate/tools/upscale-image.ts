/**
 * upscale_image tool handler (REQ-RPL-10).
 *
 * Increases image resolution via an upscaling model on Replicate.
...
export interface UpscaleImageDeps extends OutputDeps {
  eventBus: EventBus;
}
...
export function makeUpscaleImageHandler(client: ReplicateClient, deps: UpscaleImageDeps) {
  return async (args: UpscaleImageArgs): Promise<ToolResult> => {
    const start = Date.now();

    try {
      await validateInputFile(args.image);

      const model = args.model ?? getDefaultModel("upscale");
      ...
      const filename = generateFilename("upscale_image", prediction.id, ext, args.output_filename);
