...
| `daemon/services/commission-session.ts` | Commission lifecycle, in-process SDK session management, capacity limits, dependency transitions, recovery |
| `daemon/services/commission-toolbox.ts` | 3 commission tools via MCP server (report_progress, submit_result, log_question) with callback-based deps |
