...
Architectural truth / target: the daemon is the application boundary and exposes Guild Hall through REST over the Unix socket. The web layer and CLI are UX clients over that API. The daemon can spawn specialist agents, and agents interact with the application through MCP tools provided by the toolbox system. Humans interact through CLI commands and the web UI, which discover daemon operations via the OperationsRegistry. The five concerns below are daemon-internal boundaries, not alternative application boundaries. See `.lore/specs/infrastructure/daemon-application-boundary.md`.
