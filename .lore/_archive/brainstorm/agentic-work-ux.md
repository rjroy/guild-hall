...
**Worker dispatch brainstorm** (see `.lore/_abandoned/poc/poc-concepts-summary.md`) sketched the async dispatch model: fire-and-forget jobs with file-based communication. The primitives here (artifacts, workers, meetings) refine that into a more complete interaction model.
