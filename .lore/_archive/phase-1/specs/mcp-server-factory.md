...
> **Status**: This spec is superseded by the decision to use HTTP transport instead of stdio (see `.lore/_archive/phase-1/brainstorm/mcp-transport-stdio-vs-http.md`). Preserved for historical context and lessons learned. The stdio implementation was completed and reverted - code is cheap, architecture matters.
...
- **Phase I Spec** (`.lore/_archive/phase-1/specs/guild-hall-phase-1.md`): REQ-GH1-8 and REQ-GH1-29 define direct tool invocation, which requires MCPServerFactory. This spec completes the final Phase I success criterion.
- **Plugin Architecture Brainstorm** (`.lore/_archive/phase-1/brainstorm/plugin-architecture-hybrid.md`): Established the hybrid model (MCP servers for code isolation, React components for UI integration). MCPServerFactory implements the "code isolation" half.
