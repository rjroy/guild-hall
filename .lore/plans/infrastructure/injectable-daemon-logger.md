...
`apps/daemon/lib/agent-sdk/sdk-logging.ts` already uses a `log: (msg: string) => void` callback, proving the injectable approach works. That module is low priority for migration since it's already injectable and not a source of test noise.

Three test files use `spyOn(console, ...)`: `apps/daemon/lib/tests/escalation.test.ts`, `apps/daemon/tests/services/meeting/orchestrator.test.ts`, and `lib/tests/dependency-graph.test.ts`. The third is in `lib/` (out of scope per spec constraints), so only two need migration.

`apps/daemon/index.ts` is the composition root. It creates the production logger before DI is wired. Per REQ-LOG-7, this is not an exception to the rule; it's where the logger originates.

## Implementation Steps

### Step 1: Create the Log interface and three implementations

**Files**: `apps/daemon/lib/log.ts` (new), `apps/daemon/lib/tests/log.test.ts` (new)
...
In `apps/daemon/index.ts`, create a logger directly via `consoleLog("daemon")` for the handful of log calls that happen before `createProductionApp()` runs (socket binding, PID file, shutdown). These calls use the logger instance directly rather than receiving it through injection (per REQ-LOG-7 composition root rule).
...
- `apps/daemon/lib/event-bus.ts` (3 calls)
- `apps/daemon/lib/git.ts` (5 calls)
- `apps/daemon/lib/escalation.ts` (1 call)
- `apps/daemon/lib/agent-sdk/sdk-runner.ts` (2 calls) -- has two local `const log = (msg) => console.log(...)` lambdas (lines 193, 327). Replace these with `log.info` calls. Thread `Log` into `runSdkSession` and `drainSdkSession` as a parameter. Callers (commission orchestrator, meeting orchestrator) will pass their own logger in Phase 3c.
- `apps/daemon/lib/agent-sdk/sdk-logging.ts` (0 direct console calls, but takes a `log: (msg: string) => void` callback) -- Change `logSdkMessage` signature from `log: (msg: string) => void` to `log: Log`, and replace `log(msg)` calls with `log.info(msg)`. Only caller is `sdk-runner.ts` (same phase), so the change is self-contained. This eliminates the legacy callback pattern in favor of the new interface.
...
- `apps/daemon/services/workspace.ts` (8 calls) -- add `log` to `WorkspaceDeps`
- `apps/daemon/services/briefing-generator.ts` (6 calls) -- add `log` to `BriefingGeneratorDeps`
- `apps/daemon/services/memory-compaction.ts` (4 calls) -- add `log` parameter
- `apps/daemon/services/git-admin.ts` (14 calls) -- add `log` parameter to `syncProject`
- `apps/daemon/services/scheduler/index.ts` (4 calls) -- add `log` to `SchedulerDeps`
- `apps/daemon/services/meeting/record.ts` (4 calls)
- `apps/daemon/services/meeting/notes-generator.ts` (4 calls) -- add `log` to `NotesGeneratorDeps`
- `apps/daemon/services/meeting/transcript.ts` (1 call)
- `apps/daemon/services/manager/context.ts` (2 calls) -- add `log` to `ManagerContextDeps`

**Phase 3c: Orchestrator and route layer** (high-level coordination)
- `apps/daemon/services/commission/orchestrator.ts` (53 calls) -- add `log` to `CommissionOrchestratorDeps`
- `apps/daemon/services/meeting/orchestrator.ts` (39 calls) -- add `log` to `MeetingSessionDeps`
- `apps/daemon/services/manager/toolbox.ts` (26 calls) -- add `log` to `ManagerToolboxDeps`

**Phase 3d: Routes and app**
- `apps/daemon/routes/commissions.ts` (8 calls)
- `apps/daemon/routes/meetings.ts` (6 calls)
- `apps/daemon/routes/admin.ts` (5 calls)
- `apps/daemon/routes/briefing.ts` (1 call)
- `apps/daemon/app.ts` (8 calls) -- use `createLog` from deps to create tagged loggers for each service wired here

**Phase 3e: Entry point**
- `apps/daemon/index.ts` (5 calls) -- use the directly-created `consoleLog("daemon")` from Step 2
...
**Files**: `apps/daemon/lib/tests/escalation.test.ts`, `apps/daemon/tests/services/meeting/orchestrator.test.ts`
**Addresses**: REQ-LOG-6
**Expertise**: none
...
`lib/tests/dependency-graph.test.ts` is in `lib/` (out of scope per spec constraints). Leave it as-is.
