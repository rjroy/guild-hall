...
related:
  ...
  - .lore/plans/meetings/project-scoped-meetings.md
...
The `create_pr` tool and `rebase` CLI both call `hasActiveActivities()` (`apps/cli/rebase.ts:13`) to block operations when open meetings or active commissions exist for the project. This guard is correct for activity-scoped meetings, which create their own git branch. Merging the integration branch while an activity branch is still open would orphan that branch's work.
...
**Backward compatibility.** State files written before the project-scoped meetings feature don't have a `scope` field. Absent scope should be treated as `"activity"` (the original behavior), matching the convention established in `recoverMeetings()` (`apps/daemon/services/meeting/orchestrator.ts:1379`).

## Implementation Steps

### Step 1: Update `hasActiveActivities()` to skip project-scoped meetings

**File:** `apps/cli/rebase.ts:47-70`
...
Current code (`apps/cli/rebase.ts:57-62`):
...
This is the only production code change. Both callers (`makeCreatePrHandler` in `apps/daemon/services/manager/toolbox.ts:224` and `rebaseProject`/`syncProject` in `apps/cli/rebase.ts:94,212`) benefit automatically.

### Step 2: Update existing tests and add new ones

**File:** `apps/cli/tests/rebase.test.ts`
...
| File | Change | New? |
|------|--------|------|
| `apps/cli/rebase.ts` | Add `scope` check to meeting loop in `hasActiveActivities()` | No |
| `apps/cli/tests/rebase.test.ts` | Add 4 tests for scope-aware meeting filtering | No |
| `apps/daemon/tests/services/manager-toolbox.test.ts` | Add 1 test for PR creation with project-scoped meeting | No |

## Test Strategy

### Existing tests that must continue passing

- `apps/cli/tests/rebase.test.ts`: All existing `hasActiveActivities` tests (7 tests). The "returns true when an open meeting exists" test writes a state file without `scope`, which should still block (backward compat).
- `apps/cli/tests/rebase.test.ts`: All `rebaseProject` tests (5 tests). The "skips when active meeting exists" test is unaffected (no scope field = activity scope).
- `apps/cli/tests/rebase.test.ts`: All `syncProject` tests (10 tests). The skip test uses a commission, not a meeting.
- `apps/daemon/tests/services/manager-toolbox.test.ts`: The existing "blocks when active activities exist" test for `create_pr` uses a commission state file and should continue passing.

### New tests needed

| Test file | Test | Purpose |
|-----------|------|---------|
| `apps/cli/tests/rebase.test.ts` | project-scoped meeting doesn't block | Core behavior: `scope: "project"` is ignored |
| `apps/cli/tests/rebase.test.ts` | activity-scoped meeting blocks | Explicit `scope: "activity"` blocks |
| `apps/cli/tests/rebase.test.ts` | mixed scopes, activity still blocks | Both scopes present, activity wins |
| `apps/cli/tests/rebase.test.ts` | project-scoped meeting doesn't block rebase | `rebaseProject` proceeds with project-scoped meeting |
| `apps/daemon/tests/services/manager-toolbox.test.ts` | PR creation allowed with project-scoped meeting | End-to-end through the manager toolbox |
...
1. **Step 1**: Production code change (one condition added to `apps/cli/rebase.ts`).
...
Implemented exactly as planned on 2026-03-06. One production code change in `apps/cli/rebase.ts`: added `scope?: string` to the parsed meeting state type and a `state.scope !== "project"` condition to the meeting loop in `hasActiveActivities()`. Absent scope defaults to activity scope (blocking), preserving backward compatibility with pre-existing state files.

Five new tests added:
- `apps/cli/tests/rebase.test.ts`: project-scoped meeting doesn't block (3 tests covering project-only, explicit activity, and mixed scopes)
- `apps/cli/tests/rebase.test.ts`: project-scoped meeting doesn't block rebase (1 test in `rebaseProject` block)
- `apps/daemon/tests/services/manager-toolbox.test.ts`: PR creation allowed with project-scoped meeting (1 test)
