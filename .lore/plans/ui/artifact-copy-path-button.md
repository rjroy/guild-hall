...
Add a "Copy Path" button to the artifact view header that copies the artifact's `.lore/`-relative path (e.g., `.lore/specs/infrastructure/guild-hall-system.md`) to the clipboard. Clicking it during a conversation with the Guild Master lets the user paste the path directly instead of typing a title and forcing a search.
