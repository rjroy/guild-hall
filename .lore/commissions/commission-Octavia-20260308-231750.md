...
---
Updated `.lore/plans/guild-hall-scheduled-commissions.md` to account for the model selection feature (REQ-MODEL-7, REQ-MODEL-10). Changes are staged but uncommitted because the pre-commit hook fails on pre-existing lint errors in `packages/guild-hall-email/jmap-client.ts` (unrelated to this change).
