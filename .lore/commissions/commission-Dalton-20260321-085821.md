---
title: "Commission: Fix: Keep meeting input enabled during streaming"
date: 2026-03-21
status: completed
type: one-shot
tags: [commission]
worker: Dalton
workerDisplayTitle: "Guild Artificer"
prompt: "Fix the meeting chat input so users can type while the assistant is responding.\n\n**File:** `web/components/meeting/MessageInput.tsx`\n\n**What to change:**\n\n1. **Line 80:** Remove `isStreaming` from the textarea's `disabled` prop. It should be `disabled={!isOnline}` only. The user should be able to type in the textarea while streaming.\n\n2. **Line 55-59 (handleKeyDown):** Enter already guards against sending while streaming (`!isStreaming` check on line 57). This is correct. But make sure that when streaming, Enter inserts a newline instead of doing nothing. Currently it calls `e.preventDefault()` unconditionally on line 56 (before the send guard), which means during streaming Enter does nothing at all. During streaming, Enter should insert a newline (don't call `e.preventDefault()`).\n\n3. **Line 63-66 (handleSend):** The send button is replaced by Stop during streaming, so no change needed there.\n\n**Behavior after fix:**\n- While streaming: textarea is enabled, user can type, Enter inserts a newline (since the send button is Stop, not Send)\n- While not streaming: textarea is enabled, Enter sends the message (current behavior preserved)\n- While offline: textarea is disabled (current behavior preserved)\n\n**Test:** Write or update tests for the MessageInput component verifying:\n- Textarea is not disabled during streaming\n- Enter inserts newline during streaming (does not call preventDefault)\n- Enter sends message when not streaming (calls preventDefault, calls onSend)\n- Textarea is disabled when offline\n\nRun `bun test` to confirm all tests pass."
dependencies: []
linked_artifacts:
  - web/components/meeting/MessageInput.tsx

  - tests/web/components/meeting/MessageInput.test.ts
  - tests/web/components/meeting/MessageInput.test.ts
activity_timeline:
  - timestamp: 2026-03-21T15:58:21.853Z
    event: created
    reason: "Commission created"
  - timestamp: 2026-03-21T15:58:21.854Z
    event: status_dispatched
    reason: "Dispatched to worker"
    from: "pending"
    to: "dispatched"
  - timestamp: 2026-03-21T15:58:21.900Z
    event: status_in_progress
    reason: "Execution started"
    from: "dispatched"
    to: "in_progress"
  - timestamp: 2026-03-21T15:58:38.563Z
    event: progress_report
    reason: "Read MessageInput.tsx. Applying the two changes: (1) move preventDefault inside the send guard, (2) remove isStreaming from disabled prop."
  - timestamp: 2026-03-21T16:00:32.939Z
    event: result_submitted
    reason: "Fixed MessageInput so users can type while the assistant is streaming.\n\n**Changes to `web/components/meeting/MessageInput.tsx`:**\n1. Removed `isStreaming` from the textarea's `disabled` prop. Now `disabled={isTextareaDisabled({ isOnline })}` (only offline disables).\n2. Moved `e.preventDefault()` inside the send guard. During streaming, Enter falls through to the browser default (inserts newline). When not streaming with valid input, Enter prevents default and sends.\n3. Extracted `shouldSendOnEnter()` and `isTextareaDisabled()` as exported pure functions for testability.\n\n**New test file: `tests/web/components/meeting/MessageInput.test.ts`** (10 tests):\n- Textarea not disabled during streaming\n- Enter does not send (returns false for shouldSendOnEnter) during streaming, allowing newline insertion\n- Enter sends when not streaming, online, with value\n- Textarea disabled when offline\n- Edge cases: empty value, whitespace, shift key, touch device\n\nAll 3219 tests pass, typecheck clean."
  - timestamp: 2026-03-21T16:00:38.634Z
    event: status_completed
    reason: "Execution completed"
    from: "in_progress"
    to: "completed"
current_progress: "Read MessageInput.tsx. Applying the two changes: (1) move preventDefault inside the send guard, (2) remove isStreaming from disabled prop."
projectName: guild-hall
---
Fixed MessageInput so users can type while the assistant is streaming.

**Changes to `web/components/meeting/MessageInput.tsx`:**
1. Removed `isStreaming` from the textarea's `disabled` prop. Now `disabled={isTextareaDisabled({ isOnline })}` (only offline disables).
2. Moved `e.preventDefault()` inside the send guard. During streaming, Enter falls through to the browser default (inserts newline). When not streaming with valid input, Enter prevents default and sends.
3. Extracted `shouldSendOnEnter()` and `isTextareaDisabled()` as exported pure functions for testability.

**New test file: `tests/web/components/meeting/MessageInput.test.ts`** (10 tests):
- Textarea not disabled during streaming
- Enter does not send (returns false for shouldSendOnEnter) during streaming, allowing newline insertion
- Enter sends when not streaming, online, with value
- Textarea disabled when offline
- Edge cases: empty value, whitespace, shift key, touch device

All 3219 tests pass, typecheck clean.
