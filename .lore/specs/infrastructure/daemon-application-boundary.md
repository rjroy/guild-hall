...
USER NOTE: I'm not sure I reviewed this properly. Because we could define this concept like this as a general rule, but SKILL.md **must** be an outcome. With out that then nothing we are doing actually matters. So there might be two things being defined within here. (1) What the CLI/REST-API/Human/Agent see needs to have the same shape. But the way they interact with it is different. And that difference is key. An Agent knows how to interact with this because of a SKILL.md not inspite of it. I blame GPT-5 for this. This is going to have knock on affects for the rest of the system. As we've implemented the other design.
