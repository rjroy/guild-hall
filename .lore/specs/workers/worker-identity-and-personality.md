...
- [Spec: Guild Hall Workers](.lore/specs/workers/guild-hall-workers.md): Worker package API, activation contract, toolbox resolution, Agent SDK integration. REQ-WKR-2 (package metadata), REQ-WKR-3 (posture as differentiator), REQ-WKR-4 (stable identity), REQ-WKR-4a (activation function), REQ-WKR-15 (posture injected as system prompt).
- [Spec: Guild Hall Worker Roster](.lore/specs/workers/guild-hall-worker-roster.md): Five default workers, REQ-WRS-3 (shared activation pattern), REQ-WRS-4 (posture sections: Principles, Workflow, Quality Standards).
- [Spec: Guild Hall Views](.lore/specs/ui/guild-hall-views.md): REQ-VIEW-3 (consistent worker identity rendering), REQ-VIEW-28 (worker portrait and identity in meetings), REQ-VIEW-37 (provenance with worker portrait). These requirements are satisfied by existing identity metadata and are unaffected by soul.md.
