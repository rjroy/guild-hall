...
Workers have no way to know who else is in the guild or what they specialize in. A worker that needs research help has to already know that Verity exists and specializes in research. If a new worker joins the roster, no existing worker discovers this without the user mentioning it.

This spec adds a `list_guild_capabilities` read-only tool to the base toolbox. The tool returns the guild roster: who's available, what their title is, and what they do. Workers use this to make informed decisions about collaboration and to reference other workers' capabilities in their results and annotations.

## Entry Points

- Worker calls `list_guild_capabilities` during any session type (meeting, commission)
- Worker encounters a problem outside its expertise and wants to reference who else might handle it
...
- REQ-DISC-2: The tool's return format is a readable text block, not structured JSON. Workers consume this as context for understanding the guild roster, not as data to parse. Each worker entry includes the identity name, display title, and description. Example output:
...
- The tool does not expose worker posture, soul, model, or toolbox configuration. These are internal to the worker and not useful for understanding a worker's role.
- The tool does not include runtime state (active commissions, meetings). That's the `project_briefing` tool's job.

## Context

Origin: Proposal 6 in [Growth Surface brainstorm](.lore/brainstorm/growth-surface-2026-03-17.md). This spec provides a discovery layer so workers can reference other guild members' capabilities without the user having to introduce them in every prompt.
