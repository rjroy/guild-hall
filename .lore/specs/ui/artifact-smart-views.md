...
- REQ-SMARTVIEW-7: "Needs Discussion" shows artifacts where the first path segment is `brainstorm`, `issues`, or `research` AND the status matches ARTIFACT_STATUS_GROUP 0. 
- REQ-SMARTVIEW-8: "Ready to Advance" shows artifacts where the status and type signal readiness for the next lifecycle stage: status is `approved` for specs, plans, or designs. These are artifacts where the next step is commissioning them forward (spec to plan, plan to implementation).
