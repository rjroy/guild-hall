...
- The SSE event types the UI handles (text_delta, tool_use, tool_result, turn_end, error) are the daemon's cleaned-up schema, not raw SDK types. The translation layer lives in the daemon and deduplicates both text and tool_use events (the SDK emits each twice: once during streaming, once in the finalized assistant message). Tool_use events carry an `id` field and tool_result events carry a `toolUseId` field for reliable matching; the UI matches by ID first with name fallback (see REQ-MTG-15).
