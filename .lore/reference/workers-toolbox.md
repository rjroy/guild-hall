...
- **Base toolbox**: Four tools always available to every worker: `read_memory` (read from global/project/worker scopes), `edit_memory` (upsert, append, or delete named sections within a scope's single memory file), `write_memory` (deprecated alias for `edit_memory` upsert), `record_decision` (append to session decision log).
...
- **Memory injection**: Loads a single memory file per scope (global, project, worker). Each file contains named `## sections` managed by `edit_memory`. Applies a soft character cap (default 16000). Legacy multi-file layouts and `_compacted.md` files are still read for backward compatibility.
...
- **Memory files**: `~/.guild-hall/memory/global/memory.md`, `~/.guild-hall/memory/projects/<name>/memory.md`, `~/.guild-hall/memory/workers/<name>/memory.md` (single file per scope with named `## sections`)
