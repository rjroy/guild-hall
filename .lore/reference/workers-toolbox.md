...
- **Commission toolbox**: Three tools for active commissions: `report_progress` (update timeline and current progress), `submit_result` (one-shot final result with idempotency guard), `log_question` (record question in timeline). Each tool writes to files and emits EventBus events.
...
| `daemon/services/commission-toolbox.ts` | Commission toolbox factory: `report_progress`, `submit_result` (with `resultSubmitted` idempotency flag), `log_question` tools. Writes to commission artifacts and emits EventBus events. |
...
|---------|-------------|
| [commissions](./commissions.md) | Commission session resolves tools and activates workers; commission toolbox provides report_progress, submit_result, log_question |
...
- **Commission protocol injection**: The shared activation pattern injects commission-specific instructions telling the worker to use `report_progress` periodically, call `submit_result` when done, and use `log_question` for gaps. Without `submit_result`, the commission is not considered complete.
