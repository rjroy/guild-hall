...
- **Manager briefing**: AI-generated project status summary (3-5 sentences) using the Guild Master's context. Fetches on project selection, cached for 1 hour on disk. Falls back to a template-based summary when the SDK is unavailable.
...
| `daemon/services/briefing-generator.ts` | Assembles project context via `buildManagerContext()`, generates briefing via SDK (single-turn, sonnet model) or template fallback. File-based cache at `~/.guild-hall/state/briefings/<project>.json` with 1-hour TTL. |
