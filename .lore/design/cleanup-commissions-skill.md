...
How do skills attach to worker packages? Workers have posture and soul, but no mechanism for skills yet. This skill is intended for the Writer worker. The packaging question is separate from the skill definition below.

## When to Use

- After a feature's implement-review-fix cycle completes
- Before creating a PR (natural checkpoint)
- When `.lore/commissions/` has accumulated files that no longer represent active work
- Periodically, to keep the commissions directory reflecting current state
...
- The retro produced here can be graduated (lessons promoted to CLAUDE.md or rules) via the normal retro process.
- Loose threads that need tracking become `.lore/issues/`.
- If commissions reveal specs are stale relative to what was built, back-propagation can reconcile them as a separate pass.
