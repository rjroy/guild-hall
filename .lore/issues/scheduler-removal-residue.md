...
Validated 2026-04-12: all four items confirmed still present at their original locations. Production code is clean (`schedule_spawned`, `scheduleLifecycle`, `triggerEvaluator`, and `commissionType` are absent from `apps/daemon/`, `apps/web/`, and `lib/`). One additional finding added below.

**Dead type stubs.** `scheduleLifecycle` and `triggerEvaluator` fields in `apps/daemon/lib/toolbox-utils.ts:31-32` are typed as `unknown` and unused. The JSDoc comment at lines 24-25 also references these fields ("scheduleLifecycle, recordOps, and packages are optional because they're needed only for scheduled commission tools"). The comment should be updated when the fields are removed.

**Stale test: trigger route.** `apps/cli/tests/cli-error-handling.test.ts:108-109, 122-124` references `/commission/trigger/commission/update`, a route that no longer exists. These are two test cases ("optional params do not need values" and "usage line shows optional params in brackets") that use the trigger route as their fixture data. The tests themselves exercise `validateArgs` and `usageLine`, not the route, so the fix is replacing the fixture with a real route.

**Stale test: commission type.** `apps/web/tests/components/commission-view.test.tsx:115, 119` uses `commissionType: "scheduled"`, a value that no longer exists in the system. `commissionType` does not exist anywhere in production code (`apps/daemon/`, `apps/web/`), so this test validates a property that was removed entirely.
