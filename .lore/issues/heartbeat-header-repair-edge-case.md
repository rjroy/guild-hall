...
`repairHeartbeatHeader` in `apps/daemon/services/heartbeat/session.ts` assumes heartbeat files start with a level-1 heading (`# ...`). Files that start with `##` (no preceding blank line before the first section) cause content loss during header repair.
