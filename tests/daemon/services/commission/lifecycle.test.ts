...
describe("halted transitions", () => {
  // in_progress -> halted via halt()
  test("in_progress -> halted via halt()", async () => {
    lifecycle.register(TEST_ID, TEST_PROJECT, "in_progress", TEST_ARTIFACT);
    const result = await lifecycle.halt(TEST_ID, "Turn limit reached (150 turns used)");
    expect(result.outcome).toBe("executed");
    expect(lifecycle.getStatus(TEST_ID)).toBe("halted");
  });

  // halted -> in_progress via continueHalted()
  test("halted -> in_progress via continueHalted()", async () => {
    lifecycle.register(TEST_ID, TEST_PROJECT, "halted", TEST_ARTIFACT);
    const result = await lifecycle.continueHalted(TEST_ID, "Continued from halted state");
    expect(result.outcome).toBe("executed");
    expect(lifecycle.getStatus(TEST_ID)).toBe("in_progress");
  });

  // halted -> completed via executionCompleted()
  test("halted -> completed via executionCompleted()", async () => {
    lifecycle.register(TEST_ID, TEST_PROJECT, "halted", TEST_ARTIFACT);
    const result = await lifecycle.executionCompleted(TEST_ID);
    expect(result.outcome).toBe("executed");
    expect(lifecycle.getStatus(TEST_ID)).toBe("completed");
  ...
  // halted -> cancelled via cancel()
  test("halted -> cancelled via cancel()", async () => {
    lifecycle.register(TEST_ID, TEST_PROJECT, "halted", TEST_ARTIFACT);
    const result = await lifecycle.cancel(TEST_ID, "User cancelled halted commission");
    expect(result.outcome).toBe("executed");
    expect(lifecycle.getStatus(TEST_ID)).toBe("cancelled");
  });

  // halted -> abandoned via abandon()
  test("halted -> abandoned via abandon()", async () => {
    lifecycle.register(TEST_ID, TEST_PROJECT, "halted", TEST_ARTIFACT);
    const result = await lifecycle.abandon(TEST_ID, "No longer needed");
    expect(result.outcome).toBe("executed");
    expect(lifecycle.getStatus(TEST_ID)).toBe("abandoned");
  ...
  // halted -> failed via executionFailed()
  test("halted -> failed via executionFailed()", async () => {
    lifecycle.register(TEST_ID, TEST_PROJECT, "halted", TEST_ARTIFACT);
    const result = await lifecycle.executionFailed(TEST_ID, "Worktree lost during restart");
    expect(result.outcome).toBe("executed");
    expect(lifecycle.getStatus(TEST_ID)).toBe("failed");
  });

  // Invalid: halted -> pending
  test("halted -> pending via redispatch is rejected", async () => {
    lifecycle.register(TEST_ID, TEST_PROJECT, "halted", TEST_ARTIFACT);
    const result = await lifecycle.redispatch(TEST_ID);
    expect(result.outcome).toBe("skipped");
    if (result.outcome === "skipped") {
  ...
  // Invalid: halted -> dispatched
  test("halted -> dispatched is rejected", async () => {
    lifecycle.register(TEST_ID, TEST_PROJECT, "halted", TEST_ARTIFACT);
    const result = await lifecycle.dispatch(TEST_ID);
    expect(result.outcome).toBe("skipped");
  });

  // Invalid: halted -> blocked
  test("halted -> blocked is rejected", async () => {
    lifecycle.register(TEST_ID, TEST_PROJECT, "halted", TEST_ARTIFACT);
    const result = await lifecycle.block(TEST_ID);
    expect(result.outcome).toBe("skipped");
  });

  // Invalid: halted -> sleeping
  test("halted -> sleeping is rejected", async () => {
    lifecycle.register(TEST_ID, TEST_PROJECT, "halted", TEST_ARTIFACT);
    const result = await lifecycle.sleep(TEST_ID, "Cannot sleep from halted");
    expect(result.outcome).toBe("skipped");
  });

  // halt() emits commission_status event
  test("halt() emits commission_status event with reason", async () => {
    lifecycle.register(TEST_ID, TEST_PROJECT, "in_progress", TEST_ARTIFACT);
    await lifecycle.halt(TEST_ID, "Turn limit reached (150 turns used)");
    ...
    if (event.type === "commission_status") {
      ...
      expect(event.oldStatus).toBe("in_progress");
      expect(event.reason).toBe("Turn limit reached (150 turns used)");
    }
  });

  // continueHalted() emits commission_status event
  test("continueHalted() emits commission_status event with reason", async () => {
    lifecycle.register(TEST_ID, TEST_PROJECT, "halted", TEST_ARTIFACT);
    await lifecycle.continueHalted(TEST_ID, "Continued from halted state");
    ...
    if (event.type === "commission_status") {
      expect(event.commissionId).toBe(TEST_ID);
      expect(event.status).toBe("in_progress");
      expect(event.oldStatus).toBe("halted");
      expect(event.reason).toBe("Continued from halted state");
    }
  });

  // Halted commissions do NOT count as active
  test("halted commissions do not count as active", () => {
    lifecycle.register(asCommissionId("c1"), "p1", "in_progress", "/a");
    lifecycle.register(asCommissionId("c2"), "p1", "halted", "/b");
    lifecycle.register(asCommissionId("c3"), "p1", "dispatched", "/c");
  ...
  // Full halt/continue cycle
  test("full halt/continue cycle: in_progress -> halted -> in_progress -> completed", async () => {
    lifecycle.register(TEST_ID, TEST_PROJECT, "in_progress", TEST_ARTIFACT);

    await lifecycle.halt(TEST_ID, "Turn limit reached");
    expect(lifecycle.getStatus(TEST_ID)).toBe("halted");
  ...
  // Concurrent halt + halt: second is rejected
  test("concurrent halt + halt: second is rejected", async () => {
    lifecycle.register(TEST_ID, TEST_PROJECT, "in_progress", TEST_ARTIFACT);

    const [r1, r2] = await Promise.all([
      lifecycle.halt(TEST_ID, "Halt 1"),
      lifecycle.halt(TEST_ID, "Halt 2"),
    ]);
